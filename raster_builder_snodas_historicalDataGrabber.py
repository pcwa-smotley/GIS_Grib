...
@Purpose: 1) Pull data from grib file into numpy array from a series of dates going back to the 2004 water year.
...
def main():
    ...
    main_df = pd.DataFrame(columns=['Date', 'TotalAF', 'AveSWE'])
    main_df_elev = pd.DataFrame(columns=['ElevRange','TotalAF','AveSWE'])
    #for date in range(5465,-1,-1):
    for date in range(2999, -1, -1): # use 5492 for Jan 1, 2004
        try:
            deltaDay = 0
            ...
            # Debugging
            #inputArgs.date = date
            inputArgs.date = (datetime.datetime.today() - datetime.timedelta(days=date)).strftime("%Y%m%d")
            ...
            #################
            print("Pulling Data For: " + inputArgs.date + " Date Number is: " + str(date))
            ...
            # outside of both domains will still = 0. The SMUD domain will contain it's mask AND the
            ...
            if deltaDay == 0:
                df1, df_elev = excel_output(elevation_bins)
                df_elev['Date'] = grib.date.strftime("%m/%d/%Y")
                if main_df.empty:
                    main_df = df1
                else:
                    main_df = main_df.append(df1, ignore_index=True)
                if main_df_elev.empty:
                    main_df_elev = df_elev
                else:
                    main_df_elev = main_df_elev.append(df_elev, ignore_index=True)
        ...
        #except urllib.error.URLError as err:
            ...
            print("Error For Date: " + str(date) + " Error Is: " + str(err))
            continue
        if date % 500 == 0:
            main_df.to_csv('all_data'+str(date)+'.csv')
            main_df_elev.to_csv('elevation_bands' + str(date) + '.csv')
    main_df.to_csv("all_data.csv")
    main_df_elev.to_csv("elevation_bands.csv")
    return
...
def excel_output(df_elevations):
    ...
    if append:
        ...
        with pd.ExcelWriter(efile, engine='openpyxl') as writer:
            writer.book = book
