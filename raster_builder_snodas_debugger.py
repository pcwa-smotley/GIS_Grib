...
#mpl.use('tkagg') #THE ONLY WAY TO MAKE matplotlib.pyplot work.
...
def main():
    ...
    for deltaDay in comparison_days:
        ...
        grib.bbox = [-125.0,60.0,-115.0,30.0]           #[upper left lon, upper left lat, lower left lon, lower left lat]
        ...
        lons, lats = xx, yy
        #xtest_diff = np.diff(xx.flatten())
        #xtest_diff = xtest_diff[1::841]
        #ytest_diff = np.diff(yy)
        #xmax = np.max(xtest_diff)
        #xmin = np.min(xtest_diff)
        #ymax = np.max(ytest_diff)
        #ymin = np.min(ytest_diff)
        ...
        #NOTE: xres does not change for any point or latitude. yres does change by latitude, and the yres
        # given in geoinformation is the maximum yres (which will be at the equator where cos(0) = 1). As you
        # go further and further north, the yres gets smaller, so if we take the max res, then we can get the actual
        # resolution at that latitude for a given grid box.
        ...
        if inputArgs.map == True:
            ...
            #The whole globe
            #m = Basemap(ax=ax, projection='merc', llcrnrlat=-80,urcrnrlat=80,\
            ...
            # *********TEST 3: Test for grid box in boundary
            ...
            test_x = np.ma.masked_equal(debugCenterX, 0)
            test_x = test_x.compressed()
            test_y = np.ma.masked_equal(debugCenterY, 0)
            test_y = test_y.compressed()
            xs, ys = Proj(spatialRefProj)(test_x, test_y, inverse=False)
            test = np.vstack((xs,ys)).T

            #np.savetxt("testHH.csv",np.vstack((xs,ys)).T,delimiter=",")
            ...
            # and make sure to convert lat/lons to x/y's that can be used for the map => , just
            # include another line to redefine xx and yy, i.e. xx,yy = m(lons,lats))
            xc,yc = m(test_x,test_y)
            ...
            hr = 0
            xxm,yym = m(lons,lats)
            makeMap(xxm,yym, hr, m, None,df_ptVal, deltaDay, xc, yc, xres, yres)
    return
...
def createMask(xxC,yyC,spatialRefProj):
    ...
    #lons_centerPt, lats_centerPt = Proj(spatialRefProj)(xxC, yyC, inverse=True)
    lons_centerPt, lats_centerPt = xxC, yyC
    ...
    poly_area_AF = poly_area * 0.000247105 #1 sq meter to AF
    ...
    debugCenterY = np.zeros(lats_centerPt.flatten().shape)
    debugCenterX = np.zeros(lons_centerPt.flatten().shape)
...
def makeMap(lons,lats,hr,m,m2,df,deltaDay, xc, yc, xres, yres):
    ...
    im = m.pcolormesh(lons, lats, raster.T, cmap=plt.cm.jet, vmin=minVal, vmax=120) # return 50th percentile, e.g median., latlon=True)
    ...
    #testX = np.array([[xc[1]+xres/2, xc[1]+xres/2], [(xc[1]+xres/2 + 4*1323.2), (xc[1]+xres/2 + 4*1323.2)]])
    #testY = np.array([[yc[1]+yres/2, (yc[1]+yres/2 + 4*1323.2)], [yc[1]+yres/2, (yc[1] +yres/2 + 4*1323.2)]])
    testX = np.array(
        [[258930,258930], [280592,280592]])
    testY = np.array([[272599, 253390] ,[272599, 253390] ])
    testVal = np.array([[500, 500], [500, 500]])
    im1 = m.pcolormesh(testX, testY, testVal, cmap=plt.cm.jet, vmin=0.1, vmax=500, latlon=False, alpha=0.7)
    m.plot(xc[1], yc[1], 'bo')
...
def excel_output(df_elevations):
    ...
    dfe = pd.DataFrame([(grib.date.strftime("%m/%d/%Y"),grib.basinTotal, grib.basinSWE,
                         grib.SMUDbasinTotal, grib.SMUDbasinSWE)],
                       columns=['Date','TotalAF','AveSWE', 'TotalAF_SMUD','AveSWE_SMUD'])
    if grib.basin != 'Hell_Hole':
        dfe.drop(['TotalAF_SMUD','AveSWE_SMUD'], axis=1, inplace=True)
